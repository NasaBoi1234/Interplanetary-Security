...
class KickButtons(discord.ui.View):
    ...
    async def keep_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(f"✅ Approved {self.user_name} for server.", ephemeral=True)
        rules_channel = bot.get_channel(RULES_CHANNEL_ID)
        if rules_channel:
            embed = discord.Embed(
                title="User Approved",
...
async def check_member(member: discord.Member, channel: discord.TextChannel):
    ...
    if not is_old_enough:
        ...
        embed.add_field(name="Action Taken", value="**Kicked** for violating Rule 12 (account < 30 days)", inline=False)
        try:
        ...
        embed.add_field(name="Action Taken", value="**Pending manual review** - No profile picture", inline=False)
        await channel.send(embed=embed)
        ...
        embed.add_field(name="Action Taken", value="**Approved** - Member meets all requirements ✅", inline=False)
        await channel.send(embed=embed)
...
async def scanall(ctx):
    ...
    try:
            ...
            if not is_old_enough or (is_old_enough and not has_pfp):
                ...
                if private_bots:
                    ...
                    if not is_old_enough:
                        ...
                        review_embed.add_field(name="Issue", value="No profile picture", inline=False)
                    review_embed.add_field(name="Account Age", value=f"{account_age_days} days", inline=True)
        ...
        summary_embed.add_field(name="Total Members", value=f"{passed + failed}", inline=False)
        summary_embed.add_field(name="Next Steps", value=f"Check {private_bots.mention} for {failed} member(s) needing review", inline=False)
        await status_msg.edit(embed=summary_embed)

    except Exception as e:
        error_embed = discord.Embed(title="❌ Error During Scanall", description=f"Error: {str(e)}", color=discord.Color.red())
